globalThis.RSRC.get('loupe').resolve(async function () {
 ...
 const _keyValue = await load('keyValue')
 return function (config) {
  console.log('Starting loupe with config', config)
  return function ({ attachSpark, detachSpark, doc, base, launchSource }) {
   ...
   withClose({
    ...
    header(container) {
     const title = s.call(doc, 'createElement', 'button')
     s.set(title, config.sourceName, 'textContent')
     s.call(
      title,
      'addEventListener',
      'click',
      config.onClickTitle ?? launchSource
     )
     s.call(container, 'appendChild', title)
    },
    content(container) {
